...
for i in range(n_cells):
    ...
    if x_cent[i]<=0.5:
        ...
        w_cent[i] = 3.5+B_cent[i]
        u_cent[i] = 1.0
    
    else: 
 
        w_cent[i] = 0.5+B_cent[i]
        u_cent[i] = -2.0
...
np.savetxt('example2_0000.q_2d', np.transpose(q0), fmt='%19.12e') 

with open('example2_0000.q_2d','a') as file:
    file.write('\n')
...
filedata = filedata.replace('runname', 'example2')
filedata = filedata.replace('restartfile', 'example2_0000.q_2d')
